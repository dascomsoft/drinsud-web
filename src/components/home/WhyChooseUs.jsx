...
const WhyChooseUs = () => {
  const qualities = [
    {
      title: 'Innovation',
      description: 'Nous prônons au sein de notre entreprise l\'esprit d\'innovation. Ainsi, l\'entreprise reste à l\'écoute de ses employés, partenaires et clients pour améliorer ses services.'
    },
    {
      title: 'Professionalisme',
      description: 'Nos services sont guidés par le professionnalisme de notre équipe pour assurer un service de qualité à nos clients.'
    },
    {
      title: 'Transparence',
      description: 'Notre collaboration avec nos clients et partenaires est caractérisée par la transparence des transactions et services.'
    },
    {
      title: 'La confiance',
      description: 'La finalité principale de notre action est d\'établir une relation de confiance entre collaborateurs, nos partenaires et les membres de la diaspora.'
    }
  ];
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 md:mb-12" data-aos="fade-down">
          ...
          <p className="text-gray-600 text-sm sm:text-base md:text-lg max-w-2xl mx-auto">
            Découvrez la différence avec notre plateforme intégrée
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-center">
          <div className="lg:w-1/2 w-full" data-aos="fade-right">
            <div className="relative mx-auto max-w-md lg:max-w-full">
              ...
              <div className="absolute -bottom-2 -right-2 w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-orange-500 rounded-full opacity-20"></div>
              <div className="absolute -top-2 -left-2 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-blue-500 rounded-full opacity-20"></div>
            </div>
          </div>

          <div className="lg:w-1/2 w-full" data-aos="fade-left" data-aos-delay="200">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-4 md:mb-6 leading-snug">
              Explorez l'Excellence avec Nous : Votre Guichet Unique pour une Gestion Sans Compromis !
            </h3>
            <p className="text-gray-600 mb-6 md:mb-8 leading-relaxed text-sm sm:text-base">
              Découvrez la différence avec notre plateforme intégrée qui excelle dans la gestion de séjours, 
              les finances, les bâtiments, l'épargne et les projets.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              {qualities.map((quality, index) => (
                <div 
                  key={index} 
                  className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-md hover:shadow-lg transition-all duration-300 border border-gray-100"
                  data-aos="fade-up"
                  data-aos-delay={index * 100}
                >
                  <div className="flex items-start mb-3 sm:mb-4">
                    <div className="bg-orange-100 p-1.5 sm:p-2 rounded-lg mr-3 sm:mr-4 flex-shrink-0">
                      <div className="w-6 h-6 sm:w-8 sm:h-8 bg-orange-500 rounded-full flex items-center justify-center">
                        <span className="text-white font-bold text-xs sm:text-sm">{index + 1}</span>
                      </div>
                    </div>
                    <h4 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 leading-tight">{quality.title}</h4>
                  </div>
                  <p className="text-gray-600 text-xs sm:text-sm md:text-base">{quality.description}</p>
                </div>
              ))}
            </div>
          </div>
